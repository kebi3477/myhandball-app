...
`buildBadges`를 그대로 옮겼다 — 특히 **승리 요정은 무승부를 분모에 넣고**
(`attDecided = 승+무+패`), 승률 차이는 **각각 반올림한 뒤에** 뺀다.
`test/badge_service_test.dart`가 이 둘을 고정한다.
...
**시안 값이 필요하면 `spec/`을 먼저 찾고, 없으면 사용자에게 요청한다.**
추측해서 넣지 않는다.
