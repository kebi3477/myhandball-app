...
**랭킹 공개 동의는 온보딩에서 받는다** (시안 `obConsent`, 2026-09-25 개편).
닉네임 스텝에 공개 범위 표(공개 / 받지 않음 / 구분 방법)와 `(필수)` 체크박스가
붙어 있고, 체크해야 다음으로 넘어간다. 온보딩을 마치면 `goApp`처럼
**랭킹 프로필까지 만든다**(`OnboardingViewModel._createProfile`).

- 그 전에는 프로필 시트에서 `pfConsent`로 받았는데, **온보딩에서 닉네임을
  이미 정해 놓고 승부예측 탭이 또 묻는 꼴**이었다. 시안에서 `pfConsent`가
  없어졌고 앱도 따라갔다
- **프로필 생성 실패가 온보딩을 막지 않는다.** 닉네임 중복(409)이나 연결
  실패로 앱을 못 쓰게 두면 안 되고, 못 만들면 승부예측 탭이 `시작하기`
  카드를 띄워 다시 시도하게 한다
- 온보딩은 항상 다크라 공개 범위 카드가 토큰 대신 시안의 literal 색을 쓴다
  (`#222222` 배경, `#A6A6A6` 본문)

`test/onboarding_consent_test.dart`가 이 규칙들을 고정한다.
...
그 밖에 같이 맞춘 것: 셸 상단 오프라인 띠(`ApiClient.offline`),
팀 전적의 "득점 유형"
...
시안 `statOffseason`을 옮겨 뒀다가 **2026-09-25에 사용자가 지웠다.** 처음엔
네 서브탭 모두에 떠서 탭을 옮길 때마다 같은 문장이 따라다녔고, 순위 탭에만
남겨 봤지만 그래도 목록 자리만 먹었다. 시즌은 화면의 연·월 선택이 이미
말해 준다. `offseasonProvider`도 같이 지웠다 — 쓰는 곳이 없어졌다.
...
**아래 줄에 "지금은 비시즌이에요"를 붙이지 않는다.** 시안은 붙여 뒀는데
카드 맨 위가 이미 `26-27 시즌 개막까지`라서 같은 말을 두 번 하는 꼴이다
(2026-09-25에 사용자가 뺐다).
