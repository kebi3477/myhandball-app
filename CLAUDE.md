...
재료는 아직 기기에만 있어서 **재설치하면 배지도 사라진다** (가이드 진행도·
직관 기록을 서버로 옮기는 작업이 그래서 필요하다). 수료일은
`mh_guide_done_at`에 남기고, 그 전에 수료한 사용자는 날짜 없이 "가이드 완료"로
적는다.
...
**팀 상세의 "시즌 추이" 순위 모드.** `rankTrend`가 **실제 API에서 항상
비어 있다** — 연맹이 라운드별 순위를 주지 않는다. 승점 모드는 경기 결과로
직접 만들어 정상이지만, 순위 모드를 고르면 "표시할 기록이 아직 없어요"만
나온다. 목업에는 값이 있어서 프리뷰로는 멀쩡해 보이니 주의.
...
- **MY만 전체 오류 화면을 쓰지 않는다.** 마이팀·닉네임·직관 기록·배지는
  기기에 있어 연결이 끊겨도 보여줄 수 있다. `MyViewModel`이 실패한 조각만
  비우고 `MyState.error`에 원인을 담으면 `MyErrorBanner`가 위쪽에 띠로
  알리고 "다시 시도"를 준다 (시안 `hasError`)
