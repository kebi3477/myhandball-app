...
대신한다. 6칸 3열 그리드이고, **못 딴 것도 흐리게 깔고 진행바를 보여준다.**
조건은 전부 앱이 이미 가진 값으로 센다 (`domain/models/badge.dart`) — 서버에
배지 테이블을 두면 규칙 하나 고칠 때마다 배포가 필요하고 오프라인에서는
배지가 안 보인다. 대신 **재료가 기기에만 있으면 재설치할 때 배지도 사라진다**
(가이드 진행도·직관 기록을 서버로 옮기는 작업이 그래서 필요하다).
